...
console.log('Swi Service native interface vs. Swi JSON RPC interface');
console.log('----------------------------------------------------------------------');

// To run the benchmarks a fully synced Swi Core directory is needed. The RPC comands
// can be modified to match the settings in swi.conf.
...
var swid = require('../').services.Swi({
  node: {
    datadir: process.env.HOME + '/.swi',
    network: {
      name: 'testnet'
    }
  }
...
swid.start(function(err) {
  ...
  console.log('Swi Core started');
});

swid.on('ready', function() {

  console.log('Swi Core ready');

  var client = new swidRPC({
    host: 'localhost',
    port: 18332,
    user: 'swi',
    pass: 'local321'
  ...
  async.series([
    function(next) {
      ...
      function swidGetBlockNative(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
      ...
      function swidGetBlockJsonRpc(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
      ...
      function swiGetTransactionNative(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
        swid.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function swiGetTransactionJsonRpc(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
      ...
      suite.add('swid getblock (native)', swidGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('swid getblock (json rpc)', swidGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('swid gettransaction (native)', swiGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('swid gettransaction (json rpc)', swiGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
