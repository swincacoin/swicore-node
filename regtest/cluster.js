...
describe('Swi Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.swicore/data/swid')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/swi.conf'),
      rpcuser: 'swi',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/swi.conf'),
      rpcuser: 'swi',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/swi.conf'),
      rpcuser: 'swi',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 swid daemons');
    this.timeout(200000);
  ...
  it('step 1: will connect to three swid daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = swicore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.swid.once('tip', function(height) {
