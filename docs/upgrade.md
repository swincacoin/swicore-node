...
`swicore-node@2.1.1` to `swicore-node@3.0.0`
...
```json
{
  ...
  "servicesConfig": {
    "bitcoind": {
      "spawn": {
        "datadir": "/home/<username>/.bitcoin",
        "exec": "/home/<username>/swicore-node/bin/bitcoind"
      }
    }
  }
}
...
**Important**: Once changes have been made you'll also need to add the `reindex=1` option **only for the first startup** to regenerate the indexes. Once this is complete you should be able to remove the `swicore-node.db` directory with the old indexes.
