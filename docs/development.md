...
To develop swicore-node:

```bash
cd ~
git clone git@github.com:<yourusername>/swicore-node.git
...
We now will setup symlinks in `swicore-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-swi
ln -s ~/bitcoind-rpc-swi
```
...
cd devnode
mkdir node_modules
touch swicore-node.json
touch package.json
```

Edit `swicore-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../swicore-node/bin/swicore-node start
```