...
var Proposal = swicore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to swid in Node.js. Manages starting and
 * stopping swid as a child process for application support, as well as connecting
 * to multiple swid processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Swi(options) {
  if (!(this instanceof Swi)) {
    return new Swi(options);
  }
...
util.inherits(Swi, Service);

Swi.dependencies = [];

Swi.DEFAULT_MAX_TXIDS = 1000;
Swi.DEFAULT_MAX_HISTORY = 50;
Swi.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Swi.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Swi.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Swi.DEFAULT_SPAWN_RESTART_TIME = 5000;
Swi.DEFAULT_SPAWN_STOP_TIME = 10000;
Swi.DEFAULT_TRY_ALL_INTERVAL = 1000;
Swi.DEFAULT_REINDEX_INTERVAL = 10000;
Swi.DEFAULT_START_RETRY_INTERVAL = 5000;
Swi.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Swi.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Swi.DEFAULT_INSTANTSEND_FEE = 10000;
Swi.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Swi.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Swi.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Swi.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Swi.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Swi.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Swi.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Swi.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Swi.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Swi.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Swi.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Swi.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Swi.prototype._initCaches = function() {
...
Swi.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Swi.prototype.getAPIMethods = function() {
  var methods = [
...
Swi.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'swid/' + name, 'total:', this.subscriptions[name].length);
};

Swi.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'swid/' + name, 'total:', this.subscriptions[name].length);
};

Swi.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (swicore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'swid/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Swi.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'swid/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Swi.prototype.unsubscribeAddressAll = function(emitter) {
...
Swi.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Swi.DEFAULT_CONFIG_SETTINGS;
...
Swi.prototype._parseSwiConf = function(configPath) {
  var options = {};
...
Swi.prototype._expandRelativeDatadir = function() {
...
Swi.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in swid config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in swid config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in swid config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './swi.conf');

  log.info('Using Swi config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseSwiConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseSwiConf(networkConfigPath));
  }
...
Swi.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of swicore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of swicore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of swicore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in swi.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that swid is undergoing a reindex. ' +
             ...
             'of swicore-node services will start.');
    node._reindex = true;
  }
};

Swi.prototype._resetCaches = function() {
 ...
 * @private
 */
Swi.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Swi.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Swi.prototype._initChain = function(callback) {
  var self = this;
...
Swi.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === swicore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Swi.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === swicore.Networks.testnet) {
    networkPath = 'testnet3/swi.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/swi.conf';
    }
  }
  return networkPath;
};

Swi.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === swicore.Networks.testnet) {
...
Swi.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('swid/hashblock', message.toString('hex'));
    }
  }

};

Swi.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Swi.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Swi Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Swi.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Swi.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('swid/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Swi.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = swicore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('swid/rawtransaction', message.toString('hex'));
    }

    var tx = swicore.Transaction();
    tx.fromString(message);
    var txid = swicore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Swi.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = swicore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('swid/transactionlock', message.toString('hex'));
    }

  }
};

Swi.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Swi.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Swi.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Swi.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Swi.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Swi Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Swi.DEFAULT_REINDEX_INTERVAL);

};

Swi.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Swi.prototype._stopSpawnedSwi = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/swid.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned swi process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean swi process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Swi.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedSwi(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting swi process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Swi process unexpectedly exited with code:', code);
        log.warn('Restarting swi child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Swi process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn swid.'));
      }
...
Swi.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to swid.'));
    }
 ...
 */
Swi.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Swi configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Swi.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Swi.prototype.syncPercentage = function(callback) {
  var self = this;
...
Swi.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, swicore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Swi.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = swicore.Script.fromAddress(delta.address);
    return {
...
Swi.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Swi.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Swi.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in swicore is most recent to less recent
 ...
 * @param {Function} callback
 */
Swi.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Swi.prototype._getConfirmationsDetail = function(transaction) {
...
Swi.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Swi.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Swi.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A swi transaction id
 * @param {Function} callback
 */
Swi.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Swi.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof swicore.Address) {
...
Swi.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Swi.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Swi.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Swi.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Swi.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Swicore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Swi.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = swicore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Swi.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Swi.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Swi.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Swi.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Swi.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Swicore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Swi.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Swi.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new swicore.Transaction(result.hex);
      var hash = swicore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.swid.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
Swi.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Swi.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Swi.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Swi.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Swi.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Swi.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Swi.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Swi.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Swi.prototype.govCount = function (callback) {
  var self = this;
...
Swi.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Swi.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Swi.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Swi.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Swi.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.getInfo = function(callback) {
  var self = this;
...
Swi.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Swi.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('swid spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('swid process did not exit'));
      }
