...
/**
 * Will return the path and default swicore-node configuration. It will search for the
 * configuration file in the "~/.swicore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.swicore');
  var defaultConfigFile = path.resolve(defaultPath, './swicore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['swid', 'web'];
