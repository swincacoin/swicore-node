...
/**
 * Will remove a service from swicore-node.json
...
/**
 * Will remove the Node.js service and from the swicore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The swicore-node configuration directory
...
function remove(options, done) {
  ...
  var swicoreConfigPath = path.resolve(configPath, 'swicore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(swicoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a swicore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to swicore-node.json
        removeConfig(swicoreConfigPath, service, next);
      });
    }, done
  );
}
