...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for swid is necessary for this upgrade with the "reindex=1" swi.conf option. \n' +
                 'There are changes necessary in both swi.conf and swicore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/swincacoin/swicore-node/blob/swid/docs/upgrade.md \n');
        ...
        servicesConfig: {
          swid: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/swid')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed swicore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './swicore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in swicore-node services directory
    ...
    if (servicePackage.swicoreNode) {
      serviceModule = service.name + '/' + servicePackage.swicoreNode;
    }
 ...
 * @param {Node} node - The Swicore Node instance
 */
